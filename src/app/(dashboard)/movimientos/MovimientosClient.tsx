...
type Movimiento = {
  ...
  fecha: string
  tipo_movimiento: 'cuenta_cobrar' | 'abono' | 'anticipo' | 'aplicacion_saldo' | 'egreso' | 'donacion' | 'venta_externa'
...
export function MovimientosClient({ asistentes, isAdmin = false }: { asistentes: Asistente[], isAdmin?: boolean }) {
  ...
  const getTipoIcon = (tipo: string) => {
    switch (tipo) {
      ...
      case 'venta_externa': return <ArrowDownRight className="w-4 h-4 text-emerald-500" />
  ...
  const getTipoLabel = (tipo: string) => {
    switch (tipo) {
      ...
      case 'venta_externa': return 'Venta Externa'
  ...
  const getTipoBadgeColor = (tipo: string) => {
    switch (tipo) {
      ...
      case 'venta_externa': return 'bg-emerald-50 text-emerald-700 border-emerald-200'
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
          ...
          <div className="space-y-1.5">
            ...
            <select value={tipoFiltro} onChange={(e) => setTipoFiltro(e.target.value)} className="w-full h-9 rounded-md border border-[rgb(var(--border))] bg-[rgb(var(--surface-2))] px-3 py-1 text-sm text-[rgb(var(--text-primary))] focus:border-[rgb(var(--accent))] focus:outline-none focus:ring-2 focus:ring-[rgba(var(--accent),0.35)]">
              ...
              <option className="bg-[rgb(var(--surface-2))] text-[rgb(var(--text-primary))]" value="venta_externa">Ventas Externas</option>
              ...
              {!isAdmin || !isEditing || selectedMov.tipo_movimiento === 'cuenta_cobrar' ? (
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-4">
                    ...
                    {(selectedMov.tipo_movimiento === 'egreso' || selectedMov.tipo_movimiento === 'venta_externa') && (
                      <div>
                    ...
                    {(selectedMov.tipo_movimiento === 'abono' || selectedMov.tipo_movimiento === 'egreso' || selectedMov.tipo_movimiento === 'anticipo' || selectedMov.tipo_movimiento === 'donacion' || selectedMov.tipo_movimiento === 'venta_externa') && (
                      <div>
